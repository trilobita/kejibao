...
- (instancetype)initWithFrame:(CGRect)frame tabViews:(NSArray *)tabViews tabBarHeight:(CGFloat)height tabColor:(UIColor *)color backgroundColor:(UIColor *)backgroundColor {
  ...
  if (self) {
    ...
    for (UIView *tab in tabViews) {
        ...
        /*[tab setBackgroundColor:[UIColor colorWithRed:arc4random_uniform(255) / 255.0f
                                               green:arc4random_uniform(255) / 255.0f
                                                blue:arc4random_uniform(255) / 255.0f alpha:1]];
        [contentView setBackgroundColor:[UIColor colorWithRed:arc4random_uniform(255) / 255.0f
                                                        green:arc4random_uniform(255) / 255.0f
                                                         blue:arc4random_uniform(255) / 255.0f alpha:1]];*/
        
      [contentView addSubview:tab];
      [tab setTranslatesAutoresizingMaskIntoConstraints:NO];
      [VFL appendFormat:@"-%f-[T%d(%f)]", index ? 10.0f : 10.0f, index, tab.frame.size.width];
    ...
    [VFL appendString:[NSString stringWithFormat:@"-%f-|", WIDTH]];
...
//      tabIndicator.backgroundColor = [UIColor redColor];
